...
CaptureClass::CaptureClass(QMap<int, SimpleCapParams>* param)
    : gParams(param)
{
	mRefCount = 1;
	mReader = 0;
	...
	)
{
	...
	{
        if(gParams->operator[](mWhoAmI).doCapture){
			if (aSample)
			{
				...
				// Draw the frame.
                // We are not using the conversion function but write the frame in the requested format.
				if (mConvertFn)
				{
					...
					mConvertFn(
						(BYTE *)mCaptureBuffer,
                        mCaptureBufferWidth * 4, // the magical 4 is here as well.
						scanline0,
						stride,
						mCaptureBufferWidth,
				...
				int i, j;
                int *dst = (int*)gParams->operator[](mWhoAmI).mTargetBuf;
				int *src = (int*)mCaptureBuffer;
                int height = gParams->operator[](mWhoAmI).mHeight;
                int width = gParams->operator[](mWhoAmI).mWidth;
                //int bytes = gParams->operator[](mWhoAmI).mBufferBytes;
                //qDebug() << "frame h" << height << " buffer " << mCaptureBufferHeight;
                //qDebug() << "frame w" << width << " buffer " << mCaptureBufferWidth;
                //qDebug() << gParams->operator[](mWhoAmI).mBufferBytes;
                for (i = 0; i < height; i++)
				{
                    for (j = 0; j < width; j++, dst++)
					{
						*dst = src[
                            (i * mCaptureBufferHeight / height) * mCaptureBufferWidth +
                                (j * mCaptureBufferWidth / width)];
					}
				}
                gParams->operator[](mWhoAmI).doCapture = false; // wait with recording the next frame until this one is polled.
                emit newFrame(mWhoAmI);
			}
}
        else{
            qWarning() << mWhoAmI << ": Frame skipped due to slow polling";
        }
	}
...
{
    return aSubtype == mRequestedFormat;
    /*
	int i;
...
HRESULT CaptureClass::setVideoType(IMFMediaType *aType)
{
    ...
    /*
    // Probes some common formats
    qDebug() << "set isRGB32: " << (subtype == MFVideoFormat_RGB32);
    qDebug() << "set isRGB24: " << (subtype == MFVideoFormat_RGB24);
    qDebug() << "set isYUV: " << (subtype == MFVideoFormat_YUY2);
    qDebug() << "set isNV12: " << (subtype == MFVideoFormat_NV12);
    qDebug() << "set isMJPG: " << (subtype == MFVideoFormat_MJPG);
*/
    DO_OR_DIE;
    ...
    //unsigned int fourCC = MFGetAttributeUINT32(aType, MF_MT_ORIGINAL_4CC, 0);
...
HRESULT CaptureClass::initCapture(int aDevice)
{
	...
	if ((signed)param.mCount > aDevice)
	{
        ...
        mRequestedFormat = gParams->operator[](mWhoAmI).mFormat;
        int preferredmode = scanMediaTypes(gParams->operator[](mWhoAmI).mWidth, gParams->operator[](mWhoAmI).mHeight);
		mUsedIndex = preferredmode;
        //qDebug() << "Scanning media formats: " << preferredmode << " closest to " << gParams->operator[](mWhoAmI).mWidth << "x" << gParams->operator[](mWhoAmI).mHeight;
